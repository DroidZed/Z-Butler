from util.colors import CROWN_COLOR

ban_config = {
    "title": "YOU HAVE BEEN BANNED",
    "url": "https://media1.tenor.com/images/0dcb84c900e10b6272152cd759eb1eab/tenor.gif",
    "description":
        "After the actions you've done in my __server the admin decided to ban you for the safety of our community.",
    "color": CROWN_COLOR,
    "image": {
        "url": "https://media1.tenor.com/images/0dcb84c900e10b6272152cd759eb1eab/tenor.gif"},
    "author": {
        "name": "The Z Butler",
        "icon_url": "https://cdn.discordapp.com/avatars/759844892443672586/bb7df4730c048faacd8db6dd99291cdb.jpg"
    },
    "thumbnail": {
        "url": "https://64.media.tumblr.com/fbeaedb718f8f4c23d261b100bbf62cc/tumblr_onv6j3by9b1uql2i0o1_500.gif"
    },
    "footer": {
        "text": "Next time think twice before making trouble in a __server 😶",
        "icon_url": "https://emoji.gg/assets/emoji/3886_BAN.gif"
    }
}
