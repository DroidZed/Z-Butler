...
def extract_guild_data(ctx: Context, guild: Guild) -> tuple[list[Role], int, int]:
    ...
    roles: list[Role] = list(filter(lambda role: role !=
                                    ctx.guild.default_role and not role.managed, guild.roles))

    online_users_count: int = sum(list(filter(lambda member: member.status !=
                                              Status.offline and not member.bot, guild.members)))
